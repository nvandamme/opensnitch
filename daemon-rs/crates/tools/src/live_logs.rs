...
pub(crate) fn launch_daemon_live_logs() -> Result<(), DynError> {
    ...
    if env::var("OPENSNITCH_AUTOTUNE_DISABLE").is_err() {
        command.arg("OPENSNITCH_AUTOTUNE_DISABLE=1");
    }
...
pub(crate) fn run_daemon_mock_ui_live_session() -> Result<(), DynError> {
    ...
    let ready_timeout_secs = env::var("OPENSNITCH_MOCK_UI_READY_TIMEOUT_SECS")
        ...
        .unwrap_or(10)
        .max(2);
    let recap_timeout_secs = env::var("OPENSNITCH_MOCK_UI_RECAP_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(30)
        .max(2);
    #[cfg(feature = "subscriptions")]
    let mock_subscriptions = env_flag("OPENSNITCH_MOCK_UI_SUBSCRIPTIONS");
    let daemon_start_timeout_secs = env::var("OPENSNITCH_MOCK_UI_DAEMON_START_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(180)
        .max(10);
    ...
    mock_cmd
        // The checked-in Python *_pb2.py files may be generated by older protoc.
        // Force pure-Python protobuf runtime for compatibility with newer system protobuf.
    ...
    // Wait for first daemon->mock-ui traffic before starting handshake/session
    // timing windows. This keeps cargo build/startup latency out of marker timeouts.
    let daemon_start_markers: Vec<&str> = vec![
        "MOCK_UI Subscribe",
        "MOCK_UI Ping",
        "MOCK_UI Notifications stream open",
    ];

    let daemon_started = wait_for_any_log_pattern(
        &mock_stdout,
        &daemon_start_markers,
    ...
    if !daemon_started {
        ...
        stop_mock_ui_child(&mut mock_child);
        return Err(format!(
            "daemon->mock-ui startup markers were not observed within {}s (compile/startup timeout); see {} and {}",
    ...
    if !early_observed {
        eprintln!(
            "warning: full early daemon->mock-ui marker set was not observed within {}s; continuing full compatibility wait (session={}s)",
    ...
    let handshake_markers: Vec<&str> = {
        ...
        #[cfg(feature = "subscriptions")]
        if mock_subscriptions {
            markers.push("MOCK_UI SubscriptionsList");
        }
    ...
    let recap_observed = if observed {
        wait_for_log_patterns(
            &mock_stdout,
            &["MOCK_UI SessionRecap status=PASS"],
            Duration::from_secs(recap_timeout_secs),
        )
    } else {
        false
    };
    ...
    if !recap_observed {
        return Err(format!(
            "daemon->mock-ui recap marker was not observed within {}s after handshake; inspect {}",
            recap_timeout_secs,
            mock_stdout.display()
        )
        .into());
