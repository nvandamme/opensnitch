...
fn parse_proc_pid_event_exec() {
    ...
    let parsed = frame
        .parse_proc_pid_event()
        .expect("exec event should parse");
...
fn parse_proc_pid_event_exit() {
    ...
    let parsed = frame
        .parse_proc_pid_event()
        .expect("exit event should parse");
...
fn parse_proc_pid_event_fork_uses_child_pid() {
    ...
    let parsed = frame
        .parse_proc_pid_event()
        .expect("fork event should parse");
...
fn connector_payload_rejects_when_cn_len_exceeds_frame() {
    ...
    assert!(frame.connector_payload().is_none());
    assert!(frame.parse_proc_pid_event().is_none());
}
...
fn connector_read_helpers_return_none_out_of_bounds() {
    ...
    assert!(frame.read_u16_ne_at(frame.len() - 1).is_none());
    assert!(frame.read_u32_ne_at(frame.len() - 2).is_none());
}
...
fn connector_payload_requires_minimum_proc_event_header_bytes() {
    ...
    assert_eq!(
        frame.connector_payload().map(|payload| payload.len()),
