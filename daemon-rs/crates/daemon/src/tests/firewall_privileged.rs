...
async fn nftables_apply_and_clear_system_firewall_rules() {
    ...
    let _ = crate::adapters::firewall_nft::disable().await;
    let ensure_res = crate::adapters::firewall_nft::ensure(0, true).await;
    if let Err(err) = &ensure_res {
        if strict_mode() {
            panic!("nft ensure failed in strict mode: {err}");
    ...
    let apply_res = crate::adapters::firewall_nft::apply_system_firewall(&sysfw, 0).await;
    ...
    let clear_res = crate::adapters::firewall_nft::clear_system_firewall(&sysfw).await;
