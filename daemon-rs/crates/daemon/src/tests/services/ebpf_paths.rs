...
fn pin_domain_accepts_aya_override_aliases() {
    ...
    assert_eq!(resolve_pin_domain(Some("opensnitch-rs")), EbpfPinDomain::Aya);
...
fn aya_domain_uses_isolated_bpffs_roots_and_ringbuf_paths() {
    ...
    assert_eq!(EbpfPinDomain::Aya.proc_root(), "/sys/fs/bpf/opensnitch-rs/procs");
    assert_eq!(EbpfPinDomain::Aya.dns_root(), "/sys/fs/bpf/opensnitch-rs/dns");
    assert_eq!(
...
fn rust_dns_object_candidates_include_normalized_build_outputs() {
    ...
    assert!(candidates.contains(&Path::new("/workspace/daemon-rs/target/bpfel-unknown-none/release/opensnitch-ebpf").to_path_buf()));
    assert!(candidates.contains(&Path::new("/workspace/daemon-rs/target/bpfel-unknown-none/debug/opensnitch-ebpf").to_path_buf()));
}