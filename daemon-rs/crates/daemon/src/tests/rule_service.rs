...
use crate::{
    ...
    services::rule_service::{
        ListsDomainsRegexpCacheModeForTest, RuleMatchDecision, RuleService,
        load_list_entries_async_plain_for_test, measure_lists_indexing_latency_for_test,
        measure_lists_matching_latency_for_test,
    },
...
fn test_process_with_env() -> ProcessInfo {
...
async fn measure_lists_branch_indexing(
    ...
    measure_lists_indexing_latency_for_test(operand, &entries, false, mode)
}
...
async fn match_attempt_protocol_sensitive_respects_case_like_go() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.151"), &test_process(), None)
        .await?;
...
async fn match_attempt_hash_list_requires_md5_value() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.17"), &process, Some("example.org"))
        .await?;
...
async fn load_path_decodes_legacy_list_operator_data() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.18"), &test_process(), None)
        .await?;
...
async fn load_path_matches_blocklist_subscription_rule_shape_with_null_child_lists() -> Result<()> {
    ...
    let mut non_matching_uid_attempt = test_attempt("10.0.0.44");
...
async fn blocklist_subscription_rule_loads_all_txt_files_in_list_directory() -> Result<()> {
  ...
  "operator": {{
    ...
    let attempt = test_attempt("10.0.0.45");
...
async fn blocklist_large_segments_load_and_latency_smoke() -> Result<()> {
    ...
    let plain_entries = load_list_entries_async_plain_for_test(&list_dir).await?;
  ...
  "operator": {{
    ...
    let regex_index_aho = measure_lists_indexing_latency_for_test(
    ...
    let (regex_match_aho, regex_hits_aho) = measure_lists_matching_latency_for_test(
        "lists.domains_regexp",
        &regex_entries,
        false,
        "10.0.0.46",
    ...
    let regex_index_compiled = measure_lists_indexing_latency_for_test(
        "lists.domains_regexp",
        &regex_entries,
        false,
        ListsDomainsRegexpCacheModeForTest::CompiledOnly,
    ...
    let (regex_match_compiled, regex_hits_compiled) = measure_lists_matching_latency_for_test(
        "lists.domains_regexp",
        &regex_entries,
        false,
        "10.0.0.46",
...
async fn match_attempt_maps_allow_and_reject_actions() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.9"), &test_process(), None)
        .await?;
...
async fn match_attempt_returns_none_when_no_rule_matches() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.10"), &test_process(), None)
        .await?;
...
async fn match_attempt_deny_short_circuits_before_later_allow() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.11"), &test_process(), None)
        .await?;
...
async fn match_attempt_ignores_disabled_matching_rule() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.12"), &test_process(), None)
        .await?;
...
async fn match_attempt_matches_parent_process_path_operand() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.13"), &test_process(), None)
        .await?;
...
async fn match_attempt_matches_process_env_operand() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.14"), &test_process_with_env(), None)
        .await?;
...
async fn match_attempt_matches_protocol_operand() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.15"), &test_process(), None)
        .await?;
...
async fn precedence_rule_overrides_following_non_precedence_matches() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.21"), &test_process(), None)
        .await?;
...
async fn protocol_operand_is_case_insensitive_when_not_sensitive() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.22"), &test_process(), None)
        .await?;
...
async fn process_env_operand_does_not_match_when_key_is_missing() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.23"), &test_process_with_env(), None)
        .await?;
...
async fn range_operand_matches_dest_port_range() -> Result<()> {
    ...
    let decision = service
        .match_attempt(&test_attempt("10.0.0.24"), &test_process(), None)
        .await?;
