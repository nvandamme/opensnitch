...
pub(crate) trait FirewallUciCommandRunner {
    fn run_uci_cli_command(&self, command: &str) -> Result<()>;
}

impl FirewallUciCommandRunner for OpenWrtUciCommandRunner {
    fn run_uci_cli_command(&self, command: &str) -> Result<()> {
...
impl OpenWrtUciFirewallAdapter {
    ...
    pub(crate) fn build_firewall_config_cli_plan(sysfw: &FirewallConfig) -> Result<Vec<String>> {
        ...
        Self::build_firewall_persistence_plan(&raw)
    }
...
impl OpenWrtUciFirewallAdapter {
    ...
    pub(crate) fn build_firewall_persistence_plan(
    ...
    pub(crate) fn apply_cli_plan(
        commands: &[String],
        runner: &dyn FirewallUciCommandRunner,
    ) -> Result<()> {
    ...
    pub(crate) fn persist_firewall_from_uci_text(
        raw_uci_file_syntax: &str,
        runner: &dyn FirewallUciCommandRunner,
    ) -> Result<()> {
...
fn apply_openwrt_firewall_runtime(runner: &dyn FirewallUciCommandRunner) -> Result<()> {
