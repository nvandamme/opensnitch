...
use crate::platform::firewall::{
    ...
    nftables::FirewallNftablesAdapter,
};
...
async fn with_nft_netlink_timeout<T>(
    ...
    match timeout(NFT_NETLINK_REQUEST_TIMEOUT, fut).await {
        Ok(result) => result,
        Err(_) => Err(NetlinkFallbackRequired::new(
            NetlinkFallbackReason::RequestTimeout,
            format!(
...
fn mark_nft_netlink_fallback(operation: &'static str, err: &anyhow::Error) {
    ...
    let fallback_reason = err
        .downcast_ref::<NetlinkFallbackRequired>()
        .map(|fallback| fallback.reason)
        .unwrap_or(NetlinkFallbackReason::TransactionExecutionFailed);
    tracing::warn!(
