...
impl FirewallNftablesAdapter {
    ...
    fn nft_expression(rule: &FirewallRule, queue_num: u16) -> String {
        if !rule.expressions.is_empty() {
            ...
            return Self::normalize_nft_parameters(&parts.join(" "));
        }
...
impl FirewallNftablesAdapter {
    ...
    -> Result<crate::platform::firewall::port::InterceptionHealth> {
        if resolve_command_path("nft").is_none() {
            return Ok(crate::platform::firewall::port::InterceptionHealth {
                valid: false,
        ...
        Ok(crate::platform::firewall::port::InterceptionHealth {
            valid,
