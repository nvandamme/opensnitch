...
use super::decision::NfqueueDecisionState;
...
impl NfqueueVerdictEngine {
    ...
    pub(crate) fn packet_verdict_to_c(verdict: &PacketVerdict) -> (u32, u32) {
        match verdict {
            PacketVerdict::Accept { mark } => (libc::NF_ACCEPT as u32, *mark),
            PacketVerdict::AcceptWithPacket { mark, .. } => (libc::NF_ACCEPT as u32, *mark),
            PacketVerdict::Drop => (libc::NF_DROP as u32, 0),
            PacketVerdict::Requeue { queue_num, mark } => {
                ((libc::NF_QUEUE as u32) | ((*queue_num as u32) << 16), *mark)
            }
        }
    }
