...
///
/// Value from `<linux/netfilter/nf_tables.h>`:
/// ```c
...
pub(crate) fn spawn_nft_drift_listener(
    ...
    let mut sock = match MulticastSocketRaw::new(nix::libc::NETLINK_NETFILTER as u16) {
        Ok(s) => s,
        Err(err) => {
            tracing::warn!(
                "failed to open NETLINK_NETFILTER socket for nftables drift listener: {err}; \
    ...
    if let Err(err) = sock.listen(NFNLGRP_NFTABLES) {
        tracing::warn!(
            "failed to subscribe to NFNLGRP_NFTABLES (group {NFNLGRP_NFTABLES}): {err}; \
             nftables event-driven drift detection disabled"
        );
        return;
    }
