...
impl ProtoMapperAdapter {
    pub fn to_proto_process(proc_info: &ProcessInfo) -> opensnitch_proto::pb::Process {
        opensnitch_proto::pb::Process {
            ...
            env: if !proc_info.env_map.is_empty() {
            ...
            checksums: {
                ...
                if let Some(md5) = &proc_info.process_hash_md5 {
                    checksums.insert("md5".to_string(), md5.clone());
                }
                if let Some(sha1) = &proc_info.process_hash_sha1 {
                    checksums.insert("sha1".to_string(), sha1.clone());
                }
                if let Some(sha256) = &proc_info.process_hash {
