...
use crate::{
    adapters::socket_diag::SocketDiagAdapter,
    bus::Bus,
...
impl NfqueueRuntimeState {
    ...
    pub(crate) fn init(bus: Bus, primary_queue_num: u16, default_action: DefaultAction) {
...
impl NfqueuePacketParser {
    ...
    pub(crate) fn parse_dns_answer_mappings(payload: &[u8]) -> Vec<(String, String)> {
        ...
        let mut out = Vec::new();
        for _ in 0..ancount {
            let Some((answer_name, next)) = Self::parse_dns_name(dns, pos) else {
                break;
            };
            if dns.len() < next + 10 {
                break;
            ...
            match rtype {
                ...
                5 => {
                    if let Some((cname, _)) = Self::parse_dns_name(dns, rdata_off)
