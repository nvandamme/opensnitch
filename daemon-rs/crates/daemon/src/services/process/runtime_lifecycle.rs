...
impl ProcessLifecycle {
    ...
    fn current_status(&self) -> ServiceStatus {
        ServiceStatus {
            ...
            last_error: self
                ...
                .lock()
                .map(|err| err.clone())
                .unwrap_or_else(|_| Some("process lifecycle state unavailable".to_string())),
        }
    }
    ...
    fn clear_error_and_transition(&self, next_state: ServiceState) {
        self.set_error(None);
...
impl ServiceLifecycle for ProcessLifecycle {
...
#[async_trait::async_trait]
impl ServiceFactory for ProcessService {
    type FactoryInput = ();
...
#[async_trait::async_trait]
impl ServiceRuntimeControl for ProcessService {
    type ReloadInput = ();
