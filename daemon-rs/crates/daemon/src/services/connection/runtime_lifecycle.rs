...
use crate::models::connection_worker_state::ConnectionWorkerKind;

use super::connection::ConnectionRuntime;

#[tonic::async_trait]
impl ServiceLifecycle for ConnectionRuntime {
    async fn init(&mut self) -> anyhow::Result<()> {
        clear_error_and_transition(
            self,
            ConnectionRuntime::set_error,
            ConnectionRuntime::transition_state,
    ...
    async fn stop(&mut self) -> anyhow::Result<()> {
        if let Ok(mut st) = self.state.lock() {
            st.worker_kind = ConnectionWorkerKind::None;
        }
        clear_error_and_transition(
            self,
            ConnectionRuntime::set_error,
