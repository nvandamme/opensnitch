...
fn subscription_selector_id(item: &SubscriptionRecord) -> Option<String> {
    let id = item.id.trim();
    if !id.is_empty() {
...
pub(crate) fn has_refresh_targeting(items: &[SubscriptionRecord], targets: &[String]) -> bool {
    trimmed_non_empty(targets.iter().map(String::as_str))
        .next()
        .is_some()
        || items
            .iter()
...
pub(crate) fn resolve_refresh_targets(
    ...
    let mut ids: HashSet<String> = trimmed_non_empty(targets.iter().map(String::as_str))
