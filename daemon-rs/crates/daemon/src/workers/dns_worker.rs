...
impl DnsWorkerControl {
    fn set_dns_monitor_state(state: u8) {
        ...
        let label = Self::dns_monitor_state_label();
    ...
    fn spawn_worker_thread(bus: Bus, shutdown: CancellationToken) -> JoinHandle<()> {
        thread::spawn(move || {
            ...
            let ebpf_handle = thread::spawn(move || {
                Self::run_dns_ebpf_monitor(ebpf_bus, ebpf_shutdown);
            });
    ...
    #[cfg(test)]
    pub(crate) fn probe_extract_dns_events_from_varlink(value: &Value) -> Vec<(String, String)> {
        Self::extract_dns_events_from_varlink(value)
    }

    #[cfg(test)]
    pub(crate) fn probe_decode_varlink_ip(bytes: &[Value]) -> Option<String> {
...
impl OneShotWorker for DnsWorkerControl {}
...
impl DnsWorkerControl {
    fn run_dns_ebpf_monitor(_bus: Bus, shutdown: CancellationToken) {
        ...
        while !shutdown.is_cancelled() {
            Self::ensure_dns_ebpf_hook_loaded();
    ...
    fn parse_and_emit_resolved_varlink_message(bus: &Bus, value: &Value) {
        for (ip, host) in Self::extract_dns_events_from_varlink(value) {
    ...
    fn extract_dns_events_from_varlink(value: &Value) -> Vec<(String, String)> {
        ...
        for answer in answers {
            ...
            if let (Some(alias), Some(canonical)) = (key_name, cname) {
                events.push((canonical.to_string(), alias.to_string()));
            }
        }
    ...
    fn decode_varlink_ip(bytes: &[Value]) -> Option<String> {
        let parsed = bytes
            .iter()
        ...
        match parsed.len() {
            4 => Some(
                std::net::Ipv4Addr::new(parsed[0], parsed[1], parsed[2], parsed[3]).to_string(),
            ),
            16 => {
...
impl DnsEbpfRingbuf {
    fn parse_dns_ringbuf_sample(sample: &[u8]) -> Option<(String, String)> {
        const DNS_EVENT_LEN: usize = 4 + 16 + 252;
        if sample.len() != DNS_EVENT_LEN {
            return None;
        }

        let addr_type = u32::from_ne_bytes([sample[0], sample[1], sample[2], sample[3]]);
        if addr_type != 2 && addr_type != 10 {
            return None;
        }

        let ip_bytes = &sample[4..20];
        let host_bytes = &sample[20..272];
    ...
    fn try_open() -> Option<Self> {
        ...
        builder
            .add(map, move |sample: &[u8]| -> i32 {
                if let Some((ip, host)) = Self::parse_dns_ringbuf_sample(sample)
