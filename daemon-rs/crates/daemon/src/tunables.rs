...
impl RuntimeTunables {
    pub fn load_effective() -> (Self, String) {
        ...
        if let Some(path) = resolve_optin_tunables_path() {
            match load_raw_tunables(&path) {
                Ok(raw) => {
    ...
    fn apply_raw(mut self, raw: RawRuntimeTunables) -> Self {
        if let Some(value) = raw.max_concurrent_connect_attempts {
            self.max_concurrent_connect_attempts =
                clamp(value, MIN_CONNECT_WORKERS, MAX_CONNECT_WORKERS);
        }
        if let Some(value) = raw.connect_worker_queue_capacity {
            self.connect_worker_queue_capacity = clamp(
                value,
        ...
        if let Some(value) = raw.connect_dispatch_batch_size {
            self.connect_dispatch_batch_size = clamp(
                value,
        ...
        if let Some(value) = raw.kernel_dns_queue_capacity {
            self.kernel_dns_queue_capacity =
                clamp(value, MIN_KERNEL_QUEUE_CAPACITY, MAX_KERNEL_QUEUE_CAPACITY);
        }
        ...
        if let Some(value) = raw.ebpf_map_prune_threshold_percent {
            self.ebpf_map_prune_threshold_percent = clamp(
                value,
        ...
        if let Some(value) = raw.ebpf_map_prune_target_percent {
            self.ebpf_map_prune_target_percent = clamp(
                value,
    ...
    fn apply_env_overrides(&mut self) -> usize {
        ...
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_MAX_CONCURRENT_CONNECT_ATTEMPTS") {
            self.max_concurrent_connect_attempts =
                clamp(value, MIN_CONNECT_WORKERS, MAX_CONNECT_WORKERS);
            count += 1;
        }
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_CONNECT_WORKER_QUEUE_CAPACITY") {
            self.connect_worker_queue_capacity = clamp(
                value,
                MIN_CONNECT_QUEUE_CAPACITY,
                MAX_CONNECT_QUEUE_CAPACITY,
        ...
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_CONNECT_DISPATCH_BATCH_SIZE") {
            self.connect_dispatch_batch_size = clamp(
                value,
                MIN_CONNECT_DISPATCH_BATCH,
                MAX_CONNECT_DISPATCH_BATCH,
        ...
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_KERNEL_DNS_QUEUE_CAPACITY") {
            self.kernel_dns_queue_capacity =
                clamp(value, MIN_KERNEL_QUEUE_CAPACITY, MAX_KERNEL_QUEUE_CAPACITY);
            count += 1;
        }
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_KERNEL_PROCESS_QUEUE_CAPACITY") {
            self.kernel_process_queue_capacity =
                clamp(value, MIN_KERNEL_QUEUE_CAPACITY, MAX_KERNEL_QUEUE_CAPACITY);
            count += 1;
        }
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_KERNEL_FIREWALL_QUEUE_CAPACITY") {
            self.kernel_firewall_queue_capacity =
                clamp(value, MIN_KERNEL_QUEUE_CAPACITY, MAX_KERNEL_QUEUE_CAPACITY);
            count += 1;
        }
        if let Some(value) = parse_env_bool("OPENSNITCH_TUNE_EBPF_MAP_PRUNE_ENABLED") {
            self.ebpf_map_prune_enabled = value;
            count += 1;
        }
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_EBPF_MAP_PRUNE_THRESHOLD_PERCENT") {
            self.ebpf_map_prune_threshold_percent = clamp(
                value,
                MIN_EBPF_PRUNE_THRESHOLD_PERCENT,
                MAX_EBPF_PRUNE_THRESHOLD_PERCENT,
        ...
        if let Some(value) = parse_env_usize("OPENSNITCH_TUNE_EBPF_MAP_PRUNE_TARGET_PERCENT") {
            self.ebpf_map_prune_target_percent = clamp(
                value,
                MIN_EBPF_PRUNE_TARGET_PERCENT,
                MAX_EBPF_PRUNE_TARGET_PERCENT,
...
pub fn maybe_autotune_on_startup() -> Option<String> {
    ...
    if env_flag("OPENSNITCH_AUTOTUNE_DISABLE") {
        return Some("skipped(disabled)".to_string());
    }

    let output_path = resolve_tunables_output_path();
    ...
    let marker_path = resolve_autotune_marker_path(&output_path);
    ...
    if let Err(reason) = check_autotune_preflight() {
    ...
    crate::utils::systemd_notify::status("Autotuning runtime tunables before daemon readiness...");

    match run_autotune_command(&output_path, Duration::from_secs(timeout_secs)) {
        Ok(()) => {
            crate::utils::systemd_notify::status("Autotune complete; continuing daemon startup...");
            if let Err(err) = write_autotune_marker(&marker_path) {
...
fn check_autotune_preflight() -> Result<(), String> {
    ...
    let load1 = read_loadavg_1m().map_err(|err| format!("loadavg:{err}"))?;
    ...
    let mem_available_mb = read_mem_available_mb().map_err(|err| format!("meminfo:{err}"))?;
    ...
    let idle_ratio = read_cpu_idle_ratio(Duration::from_millis(sample_ms))
...
fn read_cpu_idle_ratio(sample_window: Duration) -> Result<f64, String> {
    let (idle0, total0) = read_cpu_stat_snapshot()?;
    thread::sleep(sample_window);
    let (idle1, total1) = read_cpu_stat_snapshot()?;
