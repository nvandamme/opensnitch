...
- 2026-04-27: Implemented Go `GetExtraInfo()`/`readDescriptors()` parity in pid_monitor: procfs.rs gains `read_status`, `read_stat`, `read_stack`, `ProcDescriptor`, `read_descriptors` (with socket enrichment via netlink socket-diag), `list_pids`, `list_pids_in`, `pid_owns_inode`, `pid_owns_inode_at`; `parse_socket_inode` promoted to `pub(crate)`. `ProcessExtraInfo` struct + `get_extra_info(pid)` wired into `PidMonitorResult` via `spawn_blocking`. `owner.rs` refactored to delegate to `procfs::` canonical functions; `parse_socket_inode` deduplicated (removed from `parsing.rs`). Dead wrappers removed from audit/connector/netlink-control. 6 new tests added, 594 total passing, zero warnings. `runtime_handlers.rs` grew to 980 lines (was 913); deferred split already tracked.
