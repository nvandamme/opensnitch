...
| Firewall drift detection — event-driven triggers | `workers/firewall/watch_worker.rs` (`FirewallWatchControl::targets()` returns firewall config path via `WatchWorkerControl::path_targets`) + `platform/adapters/nft_monitor.rs` (`spawn_nft_drift_listener`): inotify wakes immediately on firewall config-file writes; `NFNLGRP_NFTABLES` (group 7) multicast subscription on `NETLINK_NETFILTER` (12) triggers `heal_if_drifted()` on every kernel nftables rule-change event. Rate-limited by existing `drift_recovery_blocked_until_epoch_ms` atomic. Graceful degradation: socket-open failure logs a warning and leaves the 20 s timer as the sole fallback. | Go `daemon/firewall/` uses fsnotify for config-file reload and a ticker-based drift-heal poll; there is no equivalent NFNLGRP_NFTABLES subscription | Rust extension (beyond Go parity) | 20 s timer loop is retained as a safety-net fallback for environments where the netlink socket cannot be opened |
...
| Firewall config reload trigger model | Go `daemon/firewall/config/config.go` uses fsnotify-driven immediate reload; difference not previously called out because both sides reload configs at runtime | `services/firewall` + `services/config` rows | **Updated (v0.5.1):** Rust now uses three complementary triggers: (1) inotify on the firewall config file path via `FirewallWatchControl::targets()` + existing inotify+epoll watch infrastructure (matches Go fsnotify push-based reload); (2) `NFNLGRP_NFTABLES` (group 7) multicast subscription on `NETLINK_NETFILTER` in `platform/adapters/nft_monitor.rs` — wakes `heal_if_drifted()` immediately on any kernel nftables rule change (Rust extension beyond Go); (3) 20 s timer loop in `workers/firewall/firewall_worker.rs` as safety-net fallback. Go uses fsnotify + ticker; Rust adds netlink event subscription on top. Nft backend *operations* (ensure/disable/health/clear) remain netlink-first with nft CLI adapter fallback in both implementations |
