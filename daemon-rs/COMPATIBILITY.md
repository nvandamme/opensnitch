...
| `platform/ports/` | Internal capability interfaces for proc connector, socket diag, and firewall implementations | None (Rust-specific abstraction) | Trait-style ports used to decouple services/workers from adapter implementations | Stable | Keep ports minimal and implementation-agnostic; firewall ports accept canonical domain models and perform `pb::*` conversion only at the adapter boundary |
...
- The Go daemon has no Prometheus scrape endpoint; stats go to the UI via gRPC ping only. Rust keeps this parity by default and reserves `/metrics`-style export as an explicit feature-gated extension (`metrics-export`) via `platform::ports::stats_exporter_port::StatsExporterPort` and `StatsFlow::with_stats_exporter()`. Concrete adapters ship: **`PrometheusStatsExporter`** (Prometheus scrape with 4 format variants: Text 0.0.4, Text 1.0.0, OpenMetrics 1.0.0, protobuf — selected via content negotiation; gzip compression; SIGHUP hot-reload of listen address) and **`PushStatsExporter`** (push-gateway text/proto + InfluxDB line protocol). Configuration follows DESIGN_RULES §7: CLI switches > env vars > JSON config baseline.
