...
| `services/rule` | Rule matching and operator dispatch on connection attempts, including named network alias resolution | `daemon/rule/` | In-process LRU-backed match engine (trie + glob + regex) | Stable | No external protocol dependency; keep rule cache coherent with config reloads, explicit firewall reloads, drift-heal recovery, and nftables netlink alias-refresh triggers |
...
| `services/client` | gRPC UI client session, alert overflow queue, notification stream, and effective-session ownership attribution | `daemon/ui/client.go` + `ui/alerts.go` | tonic gRPC + ring-buffer alert queue | Stable | Multi-user session arbitration is explicit: control session takes precedence, otherwise lowest principal-rank session provides effective connected defaults and owner attribution for command/verdict mutations |
...
| `models/` | Shared contract and state layer: RPC payloads, kernel event enums, rule records, config/runtime structs, firewall domain models, task payloads, worker state snapshots | `daemon/core/` + `daemon/rule/` + `daemon/ui/` + `daemon/tasks/` + `daemon/statistics/` + `daemon/procmon/` + `daemon/firewall/` | In-process typed data contracts used across `services/`, `flows/`, `workers/`, and `platform/` | Stable | Include only parity-relevant shared contracts here; Rust now keeps a canonical `FirewallConfig` domain model and transport-neutral `CommandAction`, while Go spreads equivalent structs across domain packages |
...
| `flows/notification` | UI notification stream handling, local-principal verification, privileged command classification, and owner-scope normalization | `daemon/ui/notifications.go` + command handling embedded in `daemon/ui/client.go` | In-process VecDeque + gRPC notification stream | Stable | Rust hardening layer: transport ingress now enforces local/remote auth mode, peer identity checks, privileged-action classification, and owner-scope normalization before routing commands |
...
| `platform/netlink/` | Shared netlink runtime utility (`run_on_netlink_rt`) used by sync↔async bridge call sites in connection/NFQUEUE paths | `daemon/netlink/` shared netlink package responsibilities | `netlink-bindings` + `netlink-socket2` with shared Tokio runtime bridge | Stable | Replaces the prior generic `platform/adapters/netlink_rt.rs` surface; netlink helper now has explicit domain ownership |
| `services/storage/loadable_state.rs` | Canonical loadable-state file backend for config/rules/aliases/firewall persistence and decode via storage-format stack | Go file-backed loaders in `daemon/core/` + firewall config loaders in `daemon/firewall/config/` | StorageService + storage-format codecs (`json`/`uci`) | Stable | Replaces prior `platform/ports/loadable_state_store_port.rs` + `platform/adapters/loadable_state_file_store.rs` abstraction pair |
| `platform/ffi/` | C FFI bridge for `libnetfilter_queue` legacy compatibility path | `daemon/netfilter/` (via `libnetfilter_queue` C library) | `libc` FFI via `nix`; retained as legacy compatibility path only | Stable | Do not expand FFI surface; primary NFQUEUE runtime path is `platform/nfqueue/netlink.rs` |
...
|---|---|---|---|---|
| Explicit client authorization modes | `Config.auth_mode` with `legacy`, `local-only`, `local+remote`; config-driven local principal/group allowlists (`AllowedPrincipals`, `AllowedUsers`, `AllowedGroups`) plus remote identity binding foundations (`RemotePrincipalBindings`) for future `local+remote` enforcement | Go daemon treats the connected UI as trusted once connected; no equivalent auth-mode rollout, allowlist, or remote binding model | Rust hardening extension | Lets Rust phase in host-wide mutation controls without changing verdict-path defaults while keeping future remote authorization policy server-side |
| UI channel TLS transport security handling (`simple` / `tls-simple` / `tls-mutual`, `SkipVerify`) | `services/client/client.rs` + `services/client/transport.rs`: `Simple` uses plaintext channel; TLS modes require explicit trust material (`TLSOptions.CACert` or `TLSOptions.ServerCert`) and therefore accept maintainer/user-provided self-signed certs when configured; `TlsMutual` loads client cert/key identity; `SkipVerify=true` uses explicit custom verifier bypass with warning log; keepalive timings mirror Go (5s ping, 22s timeout); live TLS handshake capture now feeds remote principal binding resolution from the presented server certificate rather than only configured PEM files | `daemon/ui/auth/auth.go` + `daemon/ui/client.go`: `simple` uses insecure dial, TLS modes build `tls.Config` with explicit `RootCAs`, optional client cert/key, and `InsecureSkipVerify` from config; keepalive same cadence | PARITY (security baseline aligned) | **Residual minor drift (non-security):** Go threads `ClientAuthType` into client-side `tls.Config` (effectively no-op for client verification semantics), while Rust currently ignores that field. |
| Internal self-signed certificate generation utility | `main.rs` one-shot CLI + `utils/cert_gen.rs`: generates server/client self-signed PEM cert/key pairs (`--gen-self-signed-server-cert/key`, `--gen-self-signed-client-cert/key`) with optional CN/SAN/days overrides | No equivalent built-in generator in Go daemon baseline | Rust extension | Operator convenience for fail-closed TLS deployments that require explicit trust material while preserving Go-compatible runtime transport semantics once configured. |
| Local endpoint principal verification | UNIX `SO_PEERCRED` + supplementary groups, plus loopback TCP listener ownership resolution before privileged notification handling | No equivalent local peer-credential gate in baseline Go UI notification path | Rust hardening extension | Local identity is derived from OS state, not from UI-declared payload fields |
| Privileged command ingress classification | `flows/notification` classifies privileged actions as `AlwaysAllowed`, `UserScopedAllowed`, `ElevatedRequired`, `AlwaysDenied` before command dispatch | Go command path does not have an equivalent pre-dispatch authorization classifier | Rust hardening extension | Prevents transport handlers from passing unsafe client mutations into the command layer |
| Owner-scoped rule and firewall mutation compatibility | Non-root local rule/firewall mutations may proceed without elevation only when payloads are provably scoped to the authenticated caller UID/GID; Rust injects compatible owner scope where safe | Go accepts UI mutations more directly and does not implement the same owner-scope arbitration layer | Rust hardening extension | Includes `user.id` / `user.gid` rule ownership checks, iptables owner matches, nft `meta skuid` / `meta skgid`, and fail-closed conflict detection |
| Legacy ownerless rule migration workflow | `daemon/migration.rs` + CLI migration entrypoint classify and optionally rewrite legacy ownerless rules | No equivalent migration planner/write gate in baseline Go daemon | Rust hardening extension | Gives hardened modes an explicit migration path instead of silently trusting ownerless stored policy |
| Canonical flattened firewall domain model | `models/firewall_config.rs` keeps `FirewallConfig.rules` + `FirewallConfig.chains`; deprecated `pb::FwChains` wrapper is flattened at ingress and reconstructed only at wire/file egress | Go keeps protobuf/file-compatible grouped `SysFirewall.system_rules` / `FwChains` shapes as the operational model | Equivalent behavior, different internal ownership | Rust hardening/refactor extension; preserves Go/file/proto compatibility while removing wire-shape leakage from core policy/runtime code |
| Config-driven network alias source path | `Rules.NetworkAliasesFile` -> `Config.network_aliases_path` -> `RuleService` cache builder | Go uses fixed-path `network_aliases.json` loading in rule-engine alias helpers | Equivalent behavior, extended configurability | Rust keeps Go-compatible default file while allowing explicit config override |
...
| Pure-Rust NFQUEUE netlink path | `platform/nfqueue/netlink.rs` preferred path | `daemon/netfilter/queue.go` via `libnetfilter_queue` C library | Equivalent behavior, different implementation | C FFI path remains available in Rust as legacy compatibility path (`platform/ffi/`) |
...
| Event export pipeline | Concrete adapter: `platform/conman/event_logger.rs` implementing `platform/conman/event_exporter.rs::ConnectionEventExporterPort`, wired through `VerdictFlow::with_event_exporter()` in default runtime path | `daemon/log/loggers/` `LoggerManager` with `remote`/`remote_syslog`/`syslog` loggers; formats: RFC5424, RFC3164, JSON, CSV; transports: UDP, TCP | PARITY | Rust adapter ships JSON/CSV/RFC5424/RFC3164 formatting over UDP/TCP; fail-open non-blocking queue preserves verdict path latency; reconnect/backoff honors `max_connect_attempts` (`0` => indefinite); miss/default-event export parity is implemented; runtime logger-sink reconfiguration is applied by exporter refresh; local `syslog` mode now uses system syslog writer semantics |
| Multi-user session precedence + owner attribution | `services/client`: explicit principal ranking and primary-session priority for effective connected defaults; `primary_owner()` used by command and verdict transactional mutations for ownership tagging | Go tracks connected nodes and default-action config but does not expose an equivalent owner-priority transaction attribution layer | Rust hardening extension | Prevents ambiguous owner attribution across concurrent UI sessions while keeping deterministic fallback/default-action resolution |
...
| Stats snapshot export pipeline | `platform/stats/exporter_port.rs` `StatsExporterPort` trait + `StatsFlow::with_stats_exporter()` builder + concrete adapters under `platform/stats/exporters/` | No equivalent in Go daemon; stats go to UI gRPC only | Rust extension (gated `metrics-export`) | Adapters ship PrometheusText 0.0.4 / 1.0.0, OpenMetrics 1.0.0, protobuf scrape, push (text/proto/InfluxDB line protocol), gzip, content negotiation, hot-reload of scrape address on SIGHUP |
| Firewall drift detection + alias refresh triggers | `workers/firewall/watch_worker.rs` (`FirewallWatchControl::targets()` returns firewall config path via `WatchWorkerControl::path_targets`) + `platform/firewall/monitor.rs` (`spawn_nft_drift_listener`): inotify wakes immediately on firewall config-file writes; `NFNLGRP_NFTABLES` (group 7) multicast subscription on `NETLINK_NETFILTER` (12) triggers `heal_if_drifted()` on every kernel nftables rule-change event. Successful explicit firewall reloads, nftables netlink events, and drift-heal recoveries also rebuild `RuleService` match caches so `network_aliases.json` and future firewall-native alias/zone sources stay synchronized with runtime firewall state. Rate-limited by existing `drift_recovery_blocked_until_epoch_ms` atomic. Graceful degradation: socket-open failure logs a warning and leaves the 20 s timer as the sole fallback. | Go `daemon/firewall/` uses fsnotify for config-file reload and a ticker-based drift-heal poll; there is no equivalent NFNLGRP_NFTABLES subscription or rule-cache alias refresh hook | Rust extension (beyond Go parity) | 20 s timer loop is retained as a safety-net fallback for environments where the netlink socket cannot be opened |
...
- The Go daemon ships a `log/loggers/` per-connection event export pipeline (`LoggerManager`, `Remote`, `RemoteSyslog`, `Syslog` with RFC5424/RFC3164/JSON/CSV formats over UDP/TCP). Rust ships a concrete adapter at `platform/conman/event_logger.rs`, implementing `platform/conman/event_exporter.rs::ConnectionEventExporterPort` and wired into `VerdictFlow` via `with_event_exporter()` in the default runtime path.
- The Go daemon has no Prometheus scrape endpoint; stats go to the UI via gRPC ping only. Rust keeps this parity by default and reserves `/metrics`-style export as an explicit feature-gated extension (`metrics-export`) via `platform/stats/exporter_port.rs::StatsExporterPort` and `StatsFlow::with_stats_exporter()`. Concrete adapters ship: **`PrometheusStatsExporter`** (Prometheus scrape with 4 format variants: Text 0.0.4, Text 1.0.0, OpenMetrics 1.0.0, protobuf — selected via content negotiation; gzip compression; SIGHUP hot-reload of listen address) and **`PushStatsExporter`** (push-gateway text/proto + InfluxDB line protocol). Configuration follows DESIGN_RULES §7: CLI switches > env vars > JSON config baseline.
...
| Internal abstraction layer cleanup (`platform/adapters` + `platform/ports`) | Earlier phases intentionally staged decoupling seams before full domain ownership migration | `platform/netlink/`, `platform/firewall/`, `platform/procmon/`, `platform/conman/`, `platform/stats/`, `services/storage/loadable_state.rs` | Rust now uses direct domain-owned modules for active runtime surfaces; generic adapters/ports staging layers have been removed |
...
| Client-domain security model | Earlier parity passes focused on verdict/runtime behavior and under-described the trust-boundary change in UI command handling | `flows/notification`, `services/client`, extended rows `Explicit client authorization modes`, `Local endpoint principal verification`, `Owner-scoped rule and firewall mutation compatibility` | **Updated (2026-03-29):** Rust no longer treats the connected UI as implicitly trusted for host-wide mutations. It layers config-driven auth modes, OS-backed local identity verification, privileged-action classification, owner-scope normalization, and fail-closed arbitration for non-root local mutations. Go still operates with the older trusted-client model |
| Kernel capability self-check pipeline | Go `daemon/core/system.go` runs kprobe/uprobe/nfqueue/netlink/tracefs capability probes at startup; originally not mapped because Rust boot-up relied on runtime feature detection | `config.rs` + `tunables.rs` + `logging.rs` + `main.rs` row (bootstrap context) | **Updated (v0.5.1):** Rust `opensnitch-kernel-caps` crate (`crates/kernel-caps/`) reads kernel config from `/boot/config-{kver}`, `/proc/config.gz`, or `/usr/lib/modules/{kver}/config` and checks 7 feature groups (kprobes, uprobes, ftrace, syscalls, nfqueue, netlink, net diagnostics) + tracefs mount via `/proc/mounts`. Results are emitted as structured tracing events. Wired in `daemon/bootstrap.rs` mirroring Go's `core.CheckSysRequirements()`. Exposed to CLI via `--check-caps` flag in `tools` crate. **PARITY reached** with Go self-check diagnostic |
| Firewall config reload + alias refresh trigger model | Go `daemon/firewall/config/config.go` uses fsnotify-driven immediate reload and Go rule matching reads aliases from a fixed file path in `rule/operator_aliases.go`; the separation between firewall reloads and alias availability was not previously called out | `services/firewall` + `services/rule` rows; extended row `Firewall drift detection + alias refresh triggers` | **Updated (2026-03-29, refreshed 2026-04-26):** Rust now uses three complementary firewall-state triggers: (1) inotify on the firewall config file path via `FirewallWatchControl::targets()` + existing inotify+epoll watch infrastructure (matches Go fsnotify push-based reload); (2) `NFNLGRP_NFTABLES` (group 7) multicast subscription on `NETLINK_NETFILTER` in `platform/firewall/monitor.rs` — wakes `heal_if_drifted()` immediately on any kernel nftables rule change (Rust extension beyond Go); (3) 20 s timer loop in `workers/firewall/firewall_worker.rs` as safety-net fallback. Successful explicit firewall reloads, netlink events, and drift-heal recoveries also rebuild `RuleService` caches so alias inputs stay synchronized with runtime firewall state. Nft backend operations (ensure/disable/health/clear) remain netlink-first with nft CLI adapter fallback in both implementations |
| Firewall wire/domain ownership model | Earlier parity passes focused on external compatibility and missed that Rust had removed wire-shape types from core policy/runtime code | `models/` row; extended row `Canonical flattened firewall domain model` | **Updated (2026-03-29):** Rust treats `FirewallConfig` as canonical and flattens deprecated `FwChains` at ingress; Go still operates directly on grouped wire/file-compatible shapes. Behavior remains compatible, but ownership and refactor boundaries now differ intentionally |
...
|---|---|---|---|
| `platform/procmon/connector.rs` + `workers/process/netlink_worker.rs` | `procmon/process.go` + `procmon/parse.go` | 1:N | Process lifecycle ingestion path (fork/exec/exit) |
| `platform/audit/netlink.rs` + `workers/process/audit_worker.rs` | `procmon/audit/*` | 1:N | Audit event ingestion and decode path |
| `platform/netstat/socket_diag.rs` + `services/connection/connection.rs` | `netstat/find.go` + `netstat/parse.go` + `netstat/parse_packet.go` | N:N | Connection identity/socket state resolution |
| `platform/netlink/ifaces.rs` + `workers/network/netlink_addr_worker.rs` | `netlink/ifaces.go` | 1:N | Interface/address inventory and refresh |
| `platform/firewall/netlink/` + `services/firewall/firewall.rs` + `flows/verdict/verdict.rs` | `firewall/rules.go` + `netfilter/queue.go` | N:N | Verdict enforcement and rule synchronization |
| `platform/nfqueue/netlink.rs` | `netfilter/queue.go` | 1:1 (impl-different) | Primary NFQUEUE packet I/O path |
| `platform/nfqueue/ffi/` | `netfilter/queue.go` | 1:1 (fallback) | Legacy C FFI NFQUEUE compatibility path (domain-owned under nfqueue) |
