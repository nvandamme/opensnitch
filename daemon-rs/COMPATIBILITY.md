...
| `services/client` | gRPC UI client session, alert overflow queue, notification stream | `daemon/ui/client.go` + `ui/alerts.go` | tonic gRPC + ring-buffer alert queue | Stable | Keep alert ring capacity tunable via `alert_overflow_ring_capacity`; client owns UI transport |
...
| process.hash.md5 | hash compare; Go iterates ALL checksums (md5+sha1 together) and returns true if any matches or if hash unavailable; Rust checks the specific md5 field only, returns true when field is absent | PARITY | services/rule/matching.rs SimpleHashOptional + tests/rules/rule_service_match_engine.rs |
| process.hash.sha1 | same compile-time semantics as md5; Go iterates all checksums (not specific to sha1 operand); Rust checks process_hash_sha1 field specifically | PARITY | services/rule/matching.rs + tests/rules/rule_service_match_engine.rs |
...
| Hash operand checks specific field | Go's `hashCmp` iterates ALL entries in `con.Process.Checksums` (both md5 and sha1 if present) and returns true if any matches. Rust resolves the specific field (`process_hash_md5` or `process_hash_sha1`) named by the operand, and only compares that one. Both treat a missing/empty hash as a match. | services/rule/matching.rs operator_operand_value + SimpleHashOptional dispatch |
