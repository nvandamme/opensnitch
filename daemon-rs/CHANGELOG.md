...
- Began NFQUEUE netlink thin-encode hot-path compaction in `platform/nfqueue/netlink.rs`:
  ...
  - Added NFQUEUE verdict buffer reuse in hot path: `NlMsg::reuse()` clears and reuses an existing `Vec<u8>` to avoid per-verdict heap allocation, with `finalize_send_reuse()` returning the buffer after send for loop carry; `run()` loop pre-allocates and reuses `verdict_buf` across iterations (`platform/nfqueue/netlink.rs`).
  - Fixed double-alloc in firewall netlink adapter helpers: `cstr_to_string()` and `tagged_uuid_from_userdata()` now use `.into_owned()` instead of `.to_string()` when `from_utf8_lossy` returns `Cow::Borrowed`, eliminating an extra allocation for valid UTF-8 payloads (`platform/firewall/netlink/adapter.rs`).
  - Extracted `push_chain_operations()` helper to deduplicate chain+zone-chain operation-push logic in `plan_apply_system_firewall`, removing ~40 lines of copy-pasted chain iteration code and reducing per-chain `.clone()` cascade (`platform/firewall/netlink/adapter.rs`).
  - Replaced `to_ascii_lowercase().as_str()` match patterns with `eq_ignore_ascii_case()` comparisons in `chain_hook_num`, `chain_priority`, `chain_policy` to avoid per-call `String` allocation for case-insensitive matching (`platform/firewall/netlink/batch.rs`).
  - Cleaned up proc-audit dead code: removed unused `NetlinkHeader` struct, `parse_header`, `normalized_msg_len`, `align_len` private helpers; `parse_event_datagram` scoped to `#[cfg(test)]` since runtime receive path uses direct `parse_event_message`; fixed `from_utf8_lossy().to_string()` → `.to_owned()` in event data construction; removed unused `anyhow!` import (`platform/procmon/audit.rs`).
